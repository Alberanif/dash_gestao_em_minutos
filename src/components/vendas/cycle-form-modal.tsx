...
export function CycleFormModal({ products, folders = [], editTarget, onSave, onCancel, onDelete }: CycleFormModalProps) {
  const isEdit = !!editTarget;
  const [name, setName] = useState(editTarget?.name ?? "");
  const [folderId, setFolderId] = useState<string>(editTarget?.folder_id ?? "");
    ...
    async function load() {
      ...
      try {
        const res = await fetch(
          `/api/vendas/offer-options?productIds=${encodeURIComponent(missingKey)}`
        );
        if (cancelled) return;
        if (!res.ok) {
          setOffersError(true);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const url = isEdit ? `/api/vendas/cycles/${editTarget!.id}` : "/api/vendas/cycles";
      ...
      const body = isEdit
        ? productsChanged
          ? { name: name.trim(), goalPercent, status, products: productSelection, folderId: folderId || null }
          : { name: name.trim(), goalPercent, status, folderId: folderId || null }
        : { name: name.trim(), products: productSelection, goalPercent, purchasesOnly, folderId: folderId || null };
      ...
      onSave(
        { ...savedRaw, folder_id: folderId || null, products: savedProducts },
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/vendas/cycles/${editTarget!.id}`, { method: "DELETE" });
