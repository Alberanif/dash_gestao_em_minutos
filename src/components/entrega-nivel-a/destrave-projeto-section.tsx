...
import type { EntregaNivelADestraveProjetoRow } from "@/types/entrega-nivel-a";
...
export function DestraveProjetoSection() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("ena-projeto-destrave");
    if (stored) {
  ...
  function handleSelectProject(projeto: string) {
    ...
    localStorage.setItem("ena-projeto-destrave", JSON.stringify(updated));
  }
