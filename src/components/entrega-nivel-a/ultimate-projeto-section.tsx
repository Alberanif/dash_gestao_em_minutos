...
import type { EntregaNivelAUltimateProjetoRow } from "@/types/entrega-nivel-a";
...
export function UltimateProjetoSection() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("ena-projeto-ultimate");
    if (stored) {
  ...
  function handleSelectProject(projeto: string) {
    ...
    localStorage.setItem("ena-projeto-ultimate", JSON.stringify(updated));
  }
                ...
                {data ? (
                  <>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                      <MetricCell label="% Presença Encontro Online Dia 1" value={fmtPerc(data.perc_presenca_encontros_online_dia1)} />
                      <MetricCell label="% Presença Encontro Presencial" value={fmtPerc(data.perc_presenca_encontro_presencial)} />
                    </div>
                  </>
                ) : (
