...
import type { EntregaNivelAUltimateMensalRow } from "@/types/entrega-nivel-a";
...
export function UltimateMensalSection() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("ena-mensal-ultimate");
    if (stored) {
  ...
  function handleToggleMonth(monthYear: string) {
    ...
    localStorage.setItem("ena-mensal-ultimate", JSON.stringify(updated));
  }
                ...
                {data ? (
                  <>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
                      <MetricCell label="NPS Médio Entregas" value={fmtFloat(data.nps_medio_entregas)} />
                      <MetricCell label="% Presença Sessão Feedback" value={fmtPerc(data.perc_presenca_sessao_feedback)} />
                    </div>
                    <MetricCell label="MAU Usuários Ativos Mensal" value={fmtInt(data.mau_usuarios_ativos_mensal)} />
                  </>
                ) : (
