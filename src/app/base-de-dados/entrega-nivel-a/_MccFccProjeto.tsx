...
import type { EntregaNivelAMccFccProjetoRow } from "@/types/base-de-dados";
...
const FLOAT_FIELDS = ["nps", "no_show_geral"] as const;

export default function MccFccProjeto() {
  const [rows, setRows] = useState<EntregaNivelAMccFccProjetoRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [form, setForm] = useState({ projeto: "", nps: "", no_show_geral: "" });
  ...
  async function handleSave() {
    ...
    setError("");
    try {
      const res = await fetch("/api/base-de-dados/entrega-nivel-a/mcc-fcc-projeto", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          projeto: form.projeto,
          nps: parseFloat(form.nps),
          no_show_geral: parseFloat(form.no_show_geral),
        }),
      });
      if (!res.ok) {
      ...
      const savedRow: EntregaNivelAMccFccProjetoRow = await res.json();
      setRows((prev) => [savedRow, ...prev]);
      setForm({ projeto: "", nps: "", no_show_geral: "" });
      setDirty(false);
      setSaved(true);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div className="bdd-section-label">
        ...
        <span className="bdd-section-label-text">MCC e FCC — Projeto</span>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "var(--color-bg)" }}>
              ...
              <th style={{ ...thStyle, textAlign: "right" }}>No Show — Geral</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              rows.map((row) => (
                <tr key={row.id} style={{ borderTop: "1px solid var(--color-border)" }}>
                  ...
                  <td style={{ ...cellStyle, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{fmtPerc(row.no_show_geral)}</td>
                </tr>
              ))
            )}
          </tbody>
      ...
      <div style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-card)", padding: "20px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          ...
          <div>
            <label style={labelStyle}>No Show — Geral (%)</label>
            <input type="number" min={0} max={100} step={0.01} className="field-control" placeholder="0,00" value={form.no_show_geral} onChange={(e) => handleChange("no_show_geral", e.target.value)} />
          </div>
        </div>
