...
export default function Mcc() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/base-de-dados/convite/mcc").then((r) => r.json()),
      fetch("/api/convite/project-options?group=mcc").then((r) => r.json()),
    ])
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-card)", padding: "20px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div>
          ...
          >
            <option value="">
              {projectOptions.length === 0 ? "Nenhum projeto MCC cadastrado" : "Selecione um projeto"}
            </option>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div>
            ...
            <input
              ...
              className="field-control"
              placeholder="0,00"
              value={form.perc_ultimate}
              onChange={(e) => handleChange("perc_ultimate", e.target.value)}
              disabled={projectOptions.length === 0}
            />
          </div>
          <div>
