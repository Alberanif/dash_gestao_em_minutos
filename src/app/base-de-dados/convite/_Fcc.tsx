...
const FLOAT_FIELDS = ["perc_assessment", "perc_mcc", "perc_pc_ao_vivo"] as const;
...
export default function Fcc() {
  ...
  async function handleSave() {
    ...
    setError("");
    try {
      const res = await fetch("/api/base-de-dados/convite/fcc", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          projeto: form.projeto,
          perc_assessment: parseFloat(form.perc_assessment),
          perc_mcc: parseFloat(form.perc_mcc),
      ...
      const saved: FccRow = await res.json();
      setRows((prev) => [saved, ...prev]);
      setForm({ projeto: "", perc_assessment: "", perc_mcc: "", perc_pc_ao_vivo: "" });
      setDirty(false);
    } catch (err) {
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={4} style={{ ...cellStyle, textAlign: "center", color: "var(--color-text-muted)", padding: "24px 16px" }}>
              ...
              rows.map((row) => (
                <tr key={row.id} style={{ borderTop: "1px solid var(--color-border)" }}>
                  ...
                  <td style={{ ...cellStyle, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{fmtFloat(row.perc_assessment)}</td>
                  <td style={{ ...cellStyle, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{fmtFloat(row.perc_mcc)}</td>
      ...
      <div style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-card)", padding: "20px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          <div>
            ...
            <input type="number" min={0} step={0.01} style={inputStyle} placeholder="0,00" value={form.perc_assessment} onChange={(e) => handleChange("perc_assessment", e.target.value)} />
          </div>
          <div>
            <label style={labelStyle}>% MCC</label>
            <input type="number" min={0} step={0.01} style={inputStyle} placeholder="0,00" value={form.perc_mcc} onChange={(e) => handleChange("perc_mcc", e.target.value)} />
          </div>
          <div>
