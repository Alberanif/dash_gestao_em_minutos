...
export default function UltimatePercentuais() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const saved: UltimatePercentuaisRow = await res.json();
      ...
      <div style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-card)", padding: "20px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div>
            ...
            <input type="number" min={0} step={0.01} style={inputStyle} placeholder="0,00" value={form.perc_renovacao} onChange={(e) => handleChange("perc_renovacao", e.target.value)} />
          </div>
          <div>
            <label style={labelStyle}>% Conv. Pitch</label>
            <input type="number" min={0} step={0.01} style={inputStyle} placeholder="0,00" value={form.perc_conv_pitch} onChange={(e) => handleChange("perc_conv_pitch", e.target.value)} />
          </div>
        </div>
