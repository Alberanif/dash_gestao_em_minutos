...
beforeEach(() => {
  ...
  mockFrom.mockImplementation((table: string) => {
    ...
    if (table === "dash_gestao_vendas_cycle_offers") {
...
describe("GET /api/vendas/cycles", () => {
...
describe("POST /api/vendas/cycles", () => {
  it("returns 403 for role analista", async () => {
    ...
    const req = makeRequest("POST", "http://localhost/api/vendas/cycles", {
      name: "Ciclo X",
      products: [produto("p1")],
    });
  ...
  it("returns 400 when products is missing or empty", async () => {
    ...
    for (const body of [
      ...
      const res = await POST(makeRequest("POST", "http://localhost/api/vendas/cycles", body));
      expect(res.status).toBe(400);
  ...
  it("recusa produto sem oferta escolhida, nomeando quem falta", async () => {
    ...
    const res = await POST(
      makeRequest("POST", "http://localhost/api/vendas/cycles", {
        name: "Ciclo X",
        products: [
          produto("p1"),
  ...
  it("creates the cycle through the atomic RPC with deduplicated products", async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith("dash_gestao_vendas_create_cycle", {
      p_name: "Ciclo X",
      p_selection: [
        {
          product_id: "p1",
