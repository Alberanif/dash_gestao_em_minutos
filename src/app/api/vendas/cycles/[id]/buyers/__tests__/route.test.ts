...
const mockFrom = jest.fn((table: string) => {
  ...
  if (table === "dash_gestao_vendas_buyers") {
...
const URL = `http://localhost/api/vendas/cycles/${CYCLE_ID}/buyers`;
...
describe("POST /api/vendas/cycles/[id]/buyers — gates", () => {
...
describe("POST /api/vendas/cycles/[id]/buyers — mode: preview", () => {
...
describe("POST /api/vendas/cycles/[id]/buyers — mode: commit", () => {
  it("calls the atomic replace RPC exactly once with normalized rows", async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith("dash_gestao_vendas_replace_buyers", {
      p_cycle_id: CYCLE_ID,
      p_rows: [
        { email: "a@x.com", name: "Ana", phone: "123", extra: { plan: "pro" } },
