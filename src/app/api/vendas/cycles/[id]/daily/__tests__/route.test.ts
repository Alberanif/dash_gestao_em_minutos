...
describe("GET /api/vendas/cycles/[id]/daily", () => {
  it("returns 403 when role is not gestor/analista", async () => {
    ...
    const res = await GET(makeRequest("http://localhost/api/vendas/cycles/cycle-1/daily"), makeParams("cycle-1"));
  ...
  it("returns 404 when cycle does not exist", async () => {
    ...
    const { GET } = await import("../route");
    const res = await GET(makeRequest("http://localhost/api/vendas/cycles/missing/daily"), makeParams("missing"));
  ...
  it("calls the daily RPC with the cycle id and returns days", async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith("dash_gestao_vendas_daily", { p_cycle_id: "cycle-1" });
