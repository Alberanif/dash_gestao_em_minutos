...
describe("GET /api/vendas/cycles/[id]/roster", () => {
  it("returns 403 when role is not gestor/analista", async () => {
    ...
    const res = await GET(makeRequest("http://localhost/api/vendas/cycles/cycle-1/roster"), makeParams("cycle-1"));
  ...
  it("returns 404 when cycle does not exist", async () => {
    ...
    const { GET } = await import("../route");
    const res = await GET(makeRequest("http://localhost/api/vendas/cycles/missing/roster"), makeParams("missing"));
  ...
  it("calls the roster RPC with the cycle id and returns rows", async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith("dash_gestao_vendas_roster", { p_cycle_id: "cycle-1" });
...
describe("GET /api/vendas/cycles/[id]/roster — recorte por intervalo", () => {
  it("sem query string, chama a RPC só com o ciclo", async () => {
    const { GET } = await import("../route");
    await GET(
      makeRequest("http://localhost/api/vendas/cycles/cycle-1/roster"),
      makeParams("cycle-1")
    );

    expect(mockRpc).toHaveBeenCalledWith("dash_gestao_vendas_roster", {
      p_cycle_id: "cycle-1",
    });
  });
  ...
  it("repassa o intervalo à RPC quando start e end são válidos", async () => {
    ...
    const res = await GET(
      makeRequest(
        "http://localhost/api/vendas/cycles/cycle-1/roster?start=2026-07-10&end=2026-07-20"
      ),
      makeParams("cycle-1")
    );
  ...
  it("devolve 400 quando só uma ponta vem, ou quando o fim é anterior ao início", async () => {
    ...
    const soUma = await GET(
      makeRequest("http://localhost/api/vendas/cycles/cycle-1/roster?start=2026-07-10"),
      makeParams("cycle-1")
    );
    ...
    const invertido = await GET(
      makeRequest(
        "http://localhost/api/vendas/cycles/cycle-1/roster?start=2026-07-20&end=2026-07-10"
      ),
      makeParams("cycle-1")
    );
  ...
  it("devolve 501 quando a RPC não conhece os parâmetros (migration 058 pendente)", async () => {
    mockRpc.mockResolvedValueOnce({
      ...
      error: {
        code: "PGRST202",
        message:
          "Could not find the function public.dash_gestao_vendas_roster(p_cycle_id, p_end, p_start) in the schema cache",
      },
    });
...
describe("GET /api/vendas/cycles/[id]/roster — granularidade por modo", () => {
  ...
  it("ciclo Apenas Compras usa a RPC de vendas e se declara por venda", async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith("dash_gestao_vendas_purchases", {
      p_cycle_id: "cycle-1",
    });
  ...
  it("repassa o intervalo à RPC de vendas no modo Apenas Compras", async () => {
    ...
    await GET(
      makeRequest(
        "http://localhost/api/vendas/cycles/cycle-1/roster?start=2026-08-01&end=2026-08-02"
      ),
      makeParams("cycle-1")
    );
  ...
  it("devolve 501 nomeando a migration 064 quando a RPC de vendas não existe", async () => {
    ...
    mockRpc.mockResolvedValueOnce({
      ...
      error: {
        code: "PGRST202",
        message:
          "Could not find the function public.dash_gestao_vendas_purchases(p_cycle_id) in the schema cache",
      },
    });
