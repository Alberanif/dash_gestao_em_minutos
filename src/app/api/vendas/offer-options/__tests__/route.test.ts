...
function mockRpcByName(
  ...
  mockRpc.mockImplementation(async (fn: string) => {
    if (fn === "dash_gestao_vendas_offer_options") {
      return { data: offers, error: errors.offers ?? null };
    }
    if (fn === "dash_gestao_vendas_offerless_counts") {
...
describe("GET /api/vendas/offer-options", () => {
  ...
  it("escopa as duas RPCs pelos productIds, aparados e sem duplicata", async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith("dash_gestao_vendas_offer_options", {
      p_product_ids: ["p1", "p2"],
    });
    expect(mockRpc).toHaveBeenCalledWith("dash_gestao_vendas_offerless_counts", {
      p_product_ids: ["p1", "p2"],
    });
  });
