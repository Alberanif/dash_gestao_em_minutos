...
import type { UltimatesManualLinkRecord } from "@/types/vendas";
...
export async function POST(request: NextRequest) {
  ...
  // O conjunto de produtos vive em dash_gestao_vendas_cycle_products desde a
  ...
  const { data: existingLink } = await supabase
    .from("dash_gestao_vendas_manual_links")
    .select("id")
    .eq("transaction_code", transactionCode)
    .single();
...
export async function DELETE(request: NextRequest) {
  ...
  // schema (dash_gestao_vendas_manual_links só registra vínculos ativos).
  console.log(
