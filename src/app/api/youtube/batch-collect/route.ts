...
export async function POST(request: NextRequest) {
  ...
  if (accountError || !account) {
    return NextResponse.json({ error: "Conta YouTube não encontrada" }, { status: 404 });
  }

  const startedAt = new Date().toISOString();

  try {
    const result = await collectYouTube(account as Account, { start: start_date, end: end_date });
