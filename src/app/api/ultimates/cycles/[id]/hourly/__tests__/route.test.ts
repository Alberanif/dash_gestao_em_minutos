...
describe("GET /api/ultimates/cycles/[id]/hourly", () => {
  it("returns 403 when role is not gestor/analista", async () => {
    ...
    const res = await GET(makeRequest("http://localhost/api/ultimates/cycles/cycle-1/hourly"), makeParams("cycle-1"));
  ...
  it("returns 404 when cycle does not exist", async () => {
    ...
    const { GET } = await import("../route");
    const res = await GET(makeRequest("http://localhost/api/ultimates/cycles/missing/hourly"), makeParams("missing"));
  ...
  it("calls the hourly RPC with the cycle id and returns hours", async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith("dash_gestao_ultimates_hourly", { p_cycle_id: "cycle-1" });
