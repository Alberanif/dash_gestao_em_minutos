...
export default function RelacionamentoPage() {
  ...
  // YouTube derivados — views (soma do período)
  ...
  const ytPrevSparkline = ytPrevRows.map((r) => ({ date: r.date, value: r.views }));
  const ytCurrSparkline = ytCurrRows.map((r) => ({ date: r.date, value: r.views }));

  // Instagram derivados — reach (soma do período)
  ...
  const igPrevSparkline = igPrevSnaps.map((s) => ({ date: s.collected_at.slice(0, 10), value: s.reach }));
  const igCurrSparkline = igCurrSnaps.map((s) => ({ date: s.collected_at.slice(0, 10), value: s.reach }));
